...
- **распознаёт таблицы** и преобразует их в текстовый формат для RAG;
- собирает:
...
- **Text layer** (извлечённый текст PDF) — хорошо для машинного поиска, терминов, копируемых фрагментов. **Авторитетный источник при merge.**
- **Vision‑LLM распознавание скриншота страницы** — хорошо для UI‑инструкций, таблиц, схем, "визуальных" шагов. **Вспомогательный источник.**
...
3) **LLM‑распознавание (vision)**
   ...
   - Для страниц с таблицами — специализированное распознавание в Markdown‑формат.
4) **Merge (инструкция по странице)**
   - LLM объединяет распознанный текст и текстовый слой в единый «чистый» фрагмент инструкции.
   - **Приоритет отдаётся текстовому слою** — OCR используется только для заполнения пропусков.
5) **FAQ по каждой странице**
   - LLM генерирует **3–8 вопросов-ответов** (в зависимости от объёма информации) "как спросит пользователь".
   - Вопросы содержат контекст памятки (название АС/модуля) для уникальности в RAG.
6) **Сборка итогов**
...
- `page_XXX/ocr.txt` — результат OCR (или распознавания таблицы)
- `page_XXX/instruction.txt` — итоговая инструкция по странице (text layer + vision)
- `page_XXX/faq.md` — FAQ по странице
...
- **Контекст в вопросах FAQ**: каждый вопрос содержит название АС/модуля, что исключает дубли между разными инструкциями.
...
- опционально **mTLS** (клиентские сертификаты) **для текстовых запросов** (cert-first с fallback на токен); обработка изображений остаётся **токенной**

**Ключевой фокус качества**:

- объединение (merge) делается по принципу: *«текстовый слой — авторитет, OCR — вспомогательный»*;
...
- DOCX генерируется с **русским языком** (`lang=ru-RU`) и простым форматированием.
...
- `table_parser.py` — модуль для распознавания таблиц со слайдов/страниц в текстовый формат (Markdown/CSV).
- `generate_faq.py` — генерация FAQ‑вопросов по итоговой инструкции (`.md`) (3–8 вопросов на страницу в зависимости от объёма информации).
...
- `web_ui.py` — Web интерфейс: загрузка PDF → просмотр страниц (скриншот/тексты/FAQ), редактирование, показ токенов, выбор model/temperature.
...
Режим **только OCR** (без merge), где `instruction.txt` на каждой странице формируется из OCR и включает строку источника `[SOURCE -  "Памятка - NNN"]`:
...
- `page_001/ocr.txt` — результат OCR по скриншоту;
...
Для страниц, содержащих таблицы, доступен специализированный модуль распознавания:
...
Форматы вывода:
- `markdown` — Markdown-таблица (по умолчанию)
- `csv` — CSV с разделителем `;`
- `text` — текстовая таблица с рамками

### Генерация FAQ

Сгенерировать 3–8 вопросов на страницу по итоговой инструкции:
...
**Особенности генерации FAQ:**

- Количество вопросов (3–8) зависит от объёма информации на странице
- Каждый вопрос **обязательно содержит контекст памятки** (название АС/модуля/процесса) для уникальности в RAG
- Страницы без содержательной информации (титульные слайды, разделители) автоматически пропускаются
...
| Кнопка | Описание |
|--------|----------|
| **OCR только** | Распознать изображение → `ocr.txt` |
| **🗃️ Распознать таблицу** | Распознать таблицу в Markdown → `ocr.txt` |
| **Инструкция (OCR only)** | Создать инструкцию только из OCR |
| **Инструкция из текста (без OCR)** | Создать инструкцию только из текстового слоя PDF |
| **Merge (текст + OCR)** | Объединить существующий OCR и текстовый слой с приоритетом текста |
| **Обработать страницу (OCR+Merge)** | Полный цикл: новый OCR → merge → инструкция |
| **Сгенерировать FAQ** | Создать FAQ по странице |

**Редактирование:**

- Все текстовые поля (OCR, инструкция, FAQ, текстовый слой) можно редактировать прямо в браузере
- Изменения сохраняются через AJAX без перезагрузки страницы
- При повторном нажатии кнопок обработки файлы **перезаписываются** новыми значениями

**Навигация:**

- Кнопки «← Пред.» и «След. →» для быстрого перехода между страницами
- Индикатор текущей страницы: «001 / 059»

**Выгрузка:**

- Итоговая инструкция (`.md`)
- Итоговая инструкция (`.docx`, через `pandoc`, русский язык)
- FAQ по всем страницам (`.xlsx`)

**Массовые операции:**

- Загрузка **нескольких PDF** за один раз
- Массовый запуск обработки для **выбранных документов** (все страницы)
- Массовая генерация FAQ
- Web UI показывает **progressbar** (страница задания) и обновляет прогресс автоматически

### Простая обработка (OCR only)

В Web UI доступен упрощённый режим: **только OCR** по скриншотам страниц (`page.jpg → ocr.txt`), без merge.
...
Если в `.env` заданы пути к сертификатам (**mTLS**), то **текстовые** запросы могут выполняться с client‑cert. Логика приоритета для текста:
...
**Важно:** обработка изображений (upload `/files` и мультимодальные запросы с `attachments`) в этом проекте выполняется **только по Bearer‑токену**, даже если mTLS настроен.
...
Особенности:
- Основной язык документа: **русский** (`ru-RU`)
- Простое форматирование без типографских замен
...
| Переменная | Описание |
|------------|----------|
| `GIGA_ACCESS_KEY` | Авторизационный ключ для NGW |
| `GIGA_CHAT_SCOPE` | Scope для OAuth (обычно `GIGACHAT_API_CORP`) |
| `GIGA_TEXT_MODEL` | Модель для текстовых запросов |
| `GIGA_VISION_MODEL` | Модель для мультимодальных запросов |
| `GIGA_TABLE_MODEL` | Модель для распознавания таблиц (по умолчанию = VISION_MODEL) |
| `GIGA_NGW_TIMEOUT` | Таймаут запроса к NGW (секунды) |
| `GIGA_REQUEST_TIMEOUT` | Таймаут текстовых запросов к chat/completions |
...
| `PANDOC_REFERENCE_DOCX` | Reference docx для стилей |