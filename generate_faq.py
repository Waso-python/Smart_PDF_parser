...
def generate_faq_for_pages(
    ...
    sys_prompt = (
        ...
        "- ОБЯЗАТЕЛЬНО включай в формулировку вопроса КОНТЕКСТ памятки: название АС/модуля/процесса/продукта.\n"
        ...
        "о какой системе/процессе/модуле идёт речь.\n"
    )
    ...
    for page_num, page_text in pages:
        ...
        question = (
            f"НАЗВАНИЕ ПАМЯТКИ: «{pamphlet_name}»\n"
            "Используй это название (или его ключевые слова: АС, модуль, процесс, продукт) "
            "в формулировках вопросов для уникальности в RAG-поиске.\n\n"
            ...
            "Сгенерируй от 3 до 8 элементов FAQ по этой странице.\n"
            ...
            "- Каждый вопрос ОБЯЗАН содержать контекст: название АС/модуля/процесса/продукта из памятки.\n"
            "- ЗАПРЕЩЕНЫ общие формулировки: «Как создать заявку?» → ПРАВИЛЬНО: "
            f"«Как создать заявку в {pamphlet_name}?» или с упоминанием конкретного модуля/экрана.\n"
            "- Вопрос должен быть самодостаточным: читая только вопрос, должно быть понятно, о какой системе речь.\n"
            ...
            "ВОПРОС: <текст вопроса С КОНТЕКСТОМ памятки>\n\n"
